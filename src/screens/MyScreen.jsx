...
/* ───────── 마이페이지 ───────── */
export function MyScreen({score,coins,inventory,roomCards=[],ownedCount,trips,cards,room,resetDemo,apiStatus,origin,openCard,bonusActive,rushCharges,boostAdjacent,nationalActive,activeTrip,myName,avatar,onEditProfile}){
  const [showAllCards,setShowAllCards] = useState(false);
  const shownCards = showAllCards ? cards : cards.slice(0,3);
  return (<div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={S.profile}><div style={{...S.avatar, ...(avatar?{background:`url(${avatar}) center/cover no-repeat`}:{})}}>{avatar?"":"👤"}</div><div><p style={{fontFamily:"'HiKR',sans-serif",fontSize:18,color:"var(--ink)"}}>{myName || "여행자"}</p><p style={{fontSize:12,color:"var(--ink-soft)"}}>{origin?.label||"출발 지역 미설정"}{room?` · 방 ${room.code}`:""}</p></div>
      <button onClick={onEditProfile} aria-label="프로필 수정" style={S.profileEdit}>✏️</button></div>
    ...
    <Section title="개인 카드" sub={`${inventory.length}장`}>
      ...
      {(bonusActive || rushCharges>0 || boostAdjacent || nationalActive) && (
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
          ...
          {nationalActive && <span style={S.cardTag}>🗺️ 다음 주사위 전국 랜덤 적용중</span>}
          {bonusActive && <span style={S.cardTag}>⭐ 점령 보너스 적용중 · 다음 점령 점수 1.5배</span>}
